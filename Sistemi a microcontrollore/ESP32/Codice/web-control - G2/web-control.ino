...
void loop() {
  ...
  if (client) {
    ...
    if (request.indexOf("GET /temperature") >= 0) {
      ...
      // Disegna le griglie
      client.println("ctx.strokeStyle = '#ccc'; ctx.lineWidth = 1;");
      client.println("for (let i = 0; i <= 5; i++) {");
      client.println("  let y = i * (canvas.height / 5);");
      client.println("  ctx.beginPath(); ctx.moveTo(0, y); ctx.lineTo(canvas.width, y); ctx.stroke();");
      client.println("}");
